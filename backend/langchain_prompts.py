...
PREVIEW_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "Preview the match {team_a} vs {team_b}. "
...
EXPLAIN_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "A match prediction gives {prob_a_pct}% home win, "
...
MOMENTUM_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "In a match between {team_a} and {team_b}, the current prediction "
...
TACTICAL_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "Analyze the tactical matchup between {team_a} and {team_b}. "
     "{team_a} has {prob_a_pct}% win probability, draw {prob_draw_pct}%, "
     "{team_b} has {prob_b_pct}%. "
...
VAR_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "A football fan is asking about a VAR (Video Assistant Referee) decision "
...
STORY_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "Tell the story of a match between {team_a} and {team_b} "
...
LEGENDS_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "Compare two football teams from different eras: {team_a} ({era_a}) "
     "vs {team_b} ({era_b}). "
...
TEACH_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "A football fan asks: '{question}'. "
...
DOCLING_ANALYSIS_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", _sys_str("en")),
    ("human", "Analyze this football match report extracted via IBM Docling:\n\n"
     "{report_text}\n\n"
